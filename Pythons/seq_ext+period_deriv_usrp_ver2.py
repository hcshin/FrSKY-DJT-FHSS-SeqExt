...
class Sequence_extractor(gr.top_block):
    ...
    def __init__(self, addr, center_chann, chann_num_l, chann_num_r):
        ...
        self.squelch_thresholds = (-30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30, -30)
...
if __name__ == '__main__':
    parser = OptionParser(option_class=eng_option, usage="Usage: %prog arg1 val_arg1 arg2 val_arg2 ...")
    parser.add_option('-a', '--addr', dest='addr', help='address of the usrp device', metavar='usrp_addr', default='192.168.10.2')
    parser.add_option('-c', '--center', dest='center', help='ch# of the center channel', metavar='center_chann_#')
    parser.add_option('-l', '--left', dest='left', help='# of lower ch''s to be analyzed', default='8', metavar='#(lower_ch''s)')
    parser.add_option('-r', '--right', dest='right', help='# of upper ch''s to be analyzed', default='8', metavar='#(upper_ch''s)')
    parser.add_option('-t', '--runtime', dest='runtime', help='runtime duration is in sec', default='20', metavar='RUN_DUR')
    parser.add_option('-p', '--period', dest='period', help='analysis iteration period', default='1.0/500', metavar='ITER_DUR')
    (options, args) = parser.parse_args()
    
    usrp_addr = options.addr
    center_chann = int(options.center)
    left_chann = int(eval(options.left))
    right_chann = int(eval(options.right))
    runtime = eval(options.runtime)
    iter_period = eval(options.period)

    inception = dt.datetime.now()
    extractor = Sequence_extractor(usrp_addr, center_chann, left_chann, right_chann)
    ...
    #set runtime
    N_iter = int(runtime / iter_period) + 1
    ...
    for j in range(N_iter):
        for n in range(extractor.number_of_channels):
            if eval('extractor.squelch_%s.unmuted()' % n):
                channel_activity.write('%s\n' % (center_chann - left_chann + n))
        time.sleep(iter_period)
