...
# Class Members
    ...
    def rx_hop(self):
        print_elapsed('Entered self.rx_hop()', inception)
        if self.curr_chann_idx == len(self.FH_seq) - 1:
            ...
            print_elapsed('usrp: center freq is changed to %.4f' % tb.uhd_usrp_source_0.get_center_freq(), inception)
            print_elapsed('usrp_TX: center freq is changed to %.4f' % tb.uhd_usrp_sink.get_center_freq(), inception)
        else:
    ...
    def tx_hop(self):
        print_elapsed('Entered self.tx_hop()', inception)
        self.set_usrp_freq('tx')
        print_elapsed('curr chann %d' % self.FH_seq[self.curr_chann_idx], inception)
        print_elapsed('usrp: center freq is changed to %.4f' % tb.uhd_usrp_sink.get_center_freq(), inception)
        print_elapsed('usrp_TX: center freq is changed to %.4f' % tb.uhd_usrp_sink.get_center_freq(), inception)
    ...
    def initialize(self, log):
        self.log = log
        self.uhd_usrp_source_0.set_center_freq(frequency_table(self.FH_seq[self.curr_chann_idx]))
        print_elapsed('Initial center freq: chann %d' % self.uhd_usrp_source_0.get_center_freq(), inception)
...
def catch_seq(tb):
    ...
    while 1:
        ...
        print_elapsed('curr chann %d' % tb.FH_seq[tb.curr_chann_idx], inception)
        print_elapsed('Initial center freq %.4f' % tb.uhd_usrp_source_0.get_center_freq(), inception)
        tb.log.write('\n')
        ...
        tb.set_usrp_freq('rx')
        print_elapsed('curr chann %d' % tb.FH_seq[tb.curr_chann_idx], inception)
        print_elapsed('center freq is changed to %.4f' % tb.uhd_usrp_source_0.get_center_freq(), inception)
        tb.log.write('\n')
...
if __name__ == '__main__':
    ...
    log = open('./Log_controller_jammer.txt', 'w')
    ...
    catch_seq(tb)
    print_elapsed('Start Following...', inception)
    ...
    while 1:
        ...
        if not tb.get_sql_on():
            print_elapsed('RX: Waiting for the activity', inception)
            time.sleep(0.001)
        else:
            tb.tx_hop()
            tb.rx_hop()
