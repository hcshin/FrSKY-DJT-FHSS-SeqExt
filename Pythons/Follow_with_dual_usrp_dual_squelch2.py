...
# QT sink close method reimplementation
    ...
    def set_usrp_freq(self):
        self.uhd_usrp_source_0.set_center_freq(frequency_table(self.FH_seq[self.curr_chann_idx]), 0)
        while not self.uhd_usrp_source_0.get_sensor('lo_locked', 0).to_bool():
            pass
    ...
    def follow(self):
        ...
        if self.curr_chann_idx == len(self.FH_seq) - 1:
            ...
            print_elapsed(['usrp: center freq is changed to', tb.uhd_usrp_source_0.get_center_freq(0)], inception)
        else:
...
def catch_seq(tb):
    ...
    while 1:
        ...
        print_elapsed(['curr chann', tb.FH_seq[tb.curr_chann_idx]], inception)
        print_elapsed(['Initial center freq', tb.uhd_usrp_source_0.get_center_freq(0)], inception)
        print ''
        ...
        tb.set_usrp_freq()
        print_elapsed(['curr chann', tb.FH_seq[tb.curr_chann_idx]], inception)
        print_elapsed(['center freq is changed to', tb.uhd_usrp_source_0.get_center_freq(0)], inception)
        print ''
...
if __name__ == '__main__':
    ...
    tb.uhd_usrp_source_0.set_center_freq(frequency_table(tb.FH_seq[tb.curr_chann_idx]))
    print_elapsed(['Initial center freq: chann', tb.uhd_usrp_source_0.get_center_freq(0)], inception)
    print tb.uhd_usrp_source_0.get_sensor_names()
    ...
    catch_seq(tb)
    tb.blks2_selector_0.set_output_index(1)
    ...
    print 'tb.curr_chann_idx:', tb.curr_chann_idx
    tb.stop()
    tb.wait()
