...
class Sequence_extractor(gr.top_block):
    ...
    def __init__(self, center_chann, chann_num_l, chann_num_r):
        ...
        #sigular connections
        self.connect((self.blocks_file_source_0, 0), (self.throttle, 0))


        #parallel connections
        for n in range(self.number_of_channels):
            #throttle -> channel_filter -> squelch -> null_sink
            exec('self.connect((self.throttle, 0), (self.channel_filter_%s, 0), (self.squelch_%s, 0), (self.null_sink_%s, 0))' % (n, n, n))
